...
public class ItemService {
    ...
    public Page<Item> getDdbIds(PagingRequest pagingRequest) {
        ...
        if (!pagingRequest.getSearch().getValue().isEmpty()) {
            where.append("(");
            for (String field : ItemDoc.getHeader()) {
        ...
        if (!pagingRequest.getOrder().isEmpty()) {
            ...
            for (Order o : pagingRequest.getOrder()) {
                ...
                if (!ItemDoc.getHeader().contains(columnName)) {
        ...
        List<Item> ddbIds;
        if (values.isEmpty()) {
            ddbIds = jdbcTemplate.query(query.toString(), new BeanPropertyRowMapper(Item.class));
        } else {
            ddbIds = jdbcTemplate.query(query.toString(), new BeanPropertyRowMapper(Item.class), values.toArray());
        }
